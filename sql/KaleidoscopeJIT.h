...
{
  errs() << "define @" << F->getName() << "() {\n";
  for (inst_iterator I = inst_begin(F), E = inst_end(F); I != E; I++) {
    if (CallInst *C = dyn_cast<CallInst>(&*I))
      // print indirect calls or calls that are not to llvm debug stuff
  ...
  KaleidoscopeJIT()
      : TM(EngineBuilder().selectTarget()), DL(TM->createDataLayout()),
        ...
        CODLayer(OptimizeLayer,
                 ...
                 [](Function &CalledFunc) {
                    std::set<Function*> S;
                    if (CalledFunc.getName().find("JIT") != std::string::npos) {
                      Module *M = CalledFunc.getParent();
                      for (auto &F : *M) {
                      ...
                      S.insert(&CalledFunc);
                    errs() << "JIT called for " << CalledFunc.getName() << " had " << S.size() << " elements\n";
                    return S;
                  },
                 *CompileCallbackManager,
...
private:
  ...
  std::shared_ptr<Module> optimizeModule(std::shared_ptr<Module> M) {
    ...
    for (auto &F : *M) {
      //errs() << F.getName() << "\n";
      if (F.getName().find("ExecutePlanJIT") != std::string::npos) {
        //errs() << *&F << "\n";
        //printFunc(&F);
